...
- 独立设计并交付跨平台 VR 数据采集系统，涵盖 VR 头端采集、Web 后台、移动端 App、数据导出工具完整产品矩阵\allowbreak（Unity/C#/Node/Postgres/React/Flutter/Tauri）
- 基于 PICO VR SDK 开发沉浸式数据采集应用，实现空间定位、手势追踪、眼动追踪等多维数据实时采集；\allowbreak构建高并发数据接收服务，支持毫秒级数据写入与查询
...
- 主导门店 ERP 与 OA 系统开发，打通采购、库存、人事流程，提升门店运营效率 40%；\allowbreak独立开发智能音乐播放系统，实现分时段歌单自动切换
- 搭建 CI/CD 流水线，发布周期缩短 60%；建立 Code Review 机制，代码质量显著提升
...
- 独立完成产品规划、UI/UX设计、iOS客户端开发、后端服务搭建、支付对接、应用上架全流程\allowbreak（RxSwift + MVVM 架构）
- 基于高德 SDK 实现轨迹绘制与里程计算；集成科大讯飞 SDK 实现驾驶场景全语音控制；\allowbreak对接支付宝、微信、银联多渠道支付
