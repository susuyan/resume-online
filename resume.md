...
- **平台架构设计**：独立设计并交付跨平台 VR 数据采集系统，涵盖 VR 头端采集、Web 后台、移动端 App、数据导出工具完整产品矩阵（Unity/C#/Node/Postgres/React/Flutter/Tauri）
- **VR 采集模块**：基于 PICO VR SDK 开发沉浸式数据采集应用，实现空间定位、手势追踪、眼动追踪等多维数据实时采集
- **后端服务**：构建高并发数据接收服务，设计时序数据存储方案，支持毫秒级数据写入与查询
- **多端交付**：完成前端管理后台、移动端 App、跨平台桌面导出工具开发，实现数据从采集到导出的全流程闭环
...
- **全栈独立交付**：独立完成产品规划、UI/UX设计、iOS客户端开发、后端服务搭建、支付对接、应用上架全流程（RxSwift + MVVM 架构）
- **核心功能交付**：基于高德 SDK 实现轨迹绘制与里程计算；集成科大讯飞 SDK 实现驾驶场景全语音控制；对接支付宝、微信、银联多渠道支付
